...
class HigherOrderNamingGame():
    ...
    def entropy_of_simplex(self, simplex):
        """
        Function that returns the Shannon entropy of a simplex
        (list of symbols associated to the nodes composing it)
        """
        #Retrieving opinions in the simplex
        #Notice that I have to convert the frozen set of opinions {["B","A"]} as a new opinion symbol,
        #so I sort them and then I join the strings so that it becomes "AB", while it has no effects on "A" or "B"
        symbols = [''.join(sorted(list(self.opinions[n]))) for n in simplex]
        L = len(symbols)
        freqs = [f/L for f in Counter(symbols).values()]
        en = entropy(freqs)

        return en
    
    def get_avg_entropy_simplices(self):
        entropies = [self.entropy_of_simplex(simplex) for simplex in self.simplices]
    ...
    def run_with_entropies(self, path, t_max=100, check_every=10, print_every=1):
        ...
        densities_path = path + 'HONG_densities_beta%.4f_p%.2f.csv'%(self.beta, self.p)
        f = open(densities_path,'w')
        f.write('time,n_A+p,n_B,n_AB,avg_entropy_simp\n')
        ...
        while self.t <= self.t_max:
            ...
            if self.t%check_every==0:
                ...
                #computing the average entropy among all simplices
                avg_en_simp = self.get_avg_entropy_simplices()
                
                line = "%i,%.3f,%.3f,%.3f,%3f\n"%(self.t, n_Ap, n_B, n_AB, avg_en_simp)
                f.write(line)
...
for run_id in range(n_runs):
    for beta in betas:
        ...
        output_path = '../Results/Simulations/HONG_2words_Sociopatterns_entropies/%s/%s/fixed_p%.2f_varbeta_run%i/'%(dataset, rule, p, run_id)
        if not os.path.exists(output_path):
            os.makedirs(output_path)

        HONG = HigherOrderNamingGame(simplices, rule)
        HONG.SetInitialConditions(beta=beta, p=p, n_A=n_A, verbose=True)
        HONG.run_with_entropies(output_path, t_max, check_every, print_every)
